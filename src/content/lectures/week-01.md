...
![A square board with the Radiant base at the bottom-left corner and the Dire
base at the top-right. Three lanes join them: mid runs straight down the
diagonal between the bases, while the top and bottom lanes each go the long way
round along two edges. A river crosses the other diagonal and cuts mid at the
centre. A wide hatched band on all four sides marks ground that is outside the
lanes. The bottom lane is Radiant's safe lane and Dire's off lane; the top lane
is the reverse.](../../assets/diagrams/map-overview.svg)

*A schematic, not a map to scale. Note how much of the board is not lane at
all: that ground is most of where a game is actually played.*
...
| Difference | In Dota | What it changes |
| --- | --- | --- |
| **Denying** | You can kill your own creeps to keep the enemy off the experience. There is no equivalent in League. | Refusal becomes a positive action, which changes the whole feel of a lane. |
| **No recall** | The teleport scroll costs gold, has a cooldown, and takes time to channel. | Going home is an expense, and you will budget for it. |
| **The courier** | Items come to you. | "Go home to shop" and "stay in lane" are two decisions rather than one. |
