# Captured source — 7.41f mechanics

Verbatim extract of the four sections of `docs/mechanics-reference.md` that
the site states figures from. Copied by script, not retyped, so the chain
from the author's compiled source to `FACTS.md` to a page is inspectable at
every link.

`docs/mechanics-reference.md` is itself the author's restatement of
Liquipedia's *Attributes*, *Denying*, *Lane Creeps* and *Neutral Creeps*
pages, read 16 September 2026. Nothing below is reproduced from Liquipedia;
it is the course's own wording, captured unchanged from the author's file.

Sections deliberately not captured: "Roshan and Tormentor", because no page
states a figure from it, and the map facts, which live in
`docs/map-reference.md` and are geography rather than numbers.
